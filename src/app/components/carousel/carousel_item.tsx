...
export type CarouselItemParams = {
  imageUrl: string;
  alt: string;
  title: string;
  content: string;
    ...
    <div className="flex-col min-w-[370px] h-[513px] rounded-[8px] shadow-[0px_1px_3px_0px_#A6AFC366] overflow-clip">
      ...
      <div className="h-[283px] flex flex-col items-center">
        <div className="px-[35px] pt-[32px] pb-[28px]">
          <p className={typography.textLarge + " mb-[15px]"}>{title}</p>
          <p
            className={
              typography.textBase + " text-[#637381] line-clamp-[3] flex-1"
            }
          >
            {content}
          </p>
        </div>
        <button className={button.outlinedButton}>View Details</button>
      </div>
    </div>
  );
}
